...
def test_gemma_startup(context, base_url):
    ...
    login_page = GemmaLoginPage(context)
    login_page.navigate(base_url)
    expect(context, "Gemma3 is not started").to_have_title("Open WebUI")
    expect(login_page.email_input, "The email input field did not render on the screen.").to_be_visible()


def test_gemma_login(context, base_url, app_credentials):
    # Verifies that user can log in with provided credentials and google/gemma-3-4b-it is connected.
