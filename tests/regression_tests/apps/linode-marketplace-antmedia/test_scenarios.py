...
def test_antmedia_start_stream(context, base_url, app_credentials, update_codec_settings):
    # Verifies that user can start a LiveApp stream.
    username = app_credentials["Ant Media Server Username"]
    password = app_credentials["Ant Media Server Password"]
    login_page = LoginPage(context)
    login_page.navigate(base_url)
    login_page.login(username, password)
    dashboard_page = DashboardPage(context)
    web_rtc_stream_url = f"{base_url}/LiveApp/index.html"
    new_page = dashboard_page.open_new_tab()
    stream_page = StreamPage(new_page)
    stream_page.navigate(web_rtc_stream_url)
    expect(stream_page.stream_frame.status_offline_label, "LiveApp is not available").to_be_visible()
