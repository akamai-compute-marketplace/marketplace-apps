...
def test_gpt_startup(context, base_url):
    ...
    login_page = GPTLoginPage(context)
    login_page.navigate(base_url)
    expect(context, "GPT-OSS is not started").to_have_title("Open WebUI")
    expect(login_page.email_input, "The email input field did not render on the screen.").to_be_visible()


def test_gpt_login(context, base_url, app_credentials):
    # Verifies that user can log in with provided credentials and openai/gpt-oss-20b is connected.
