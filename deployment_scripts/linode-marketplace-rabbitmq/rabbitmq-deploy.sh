...
#github_endpoint: 'https://raw.githubusercontent.com/akamai-compute-marketplace/marketplace-apps/main/deployment_scripts/linode-marketplace-rabbitmq/rabbitmq-deploy.sh'
...
#<UDF name="rabbitmq_username" label="RabbitMQ limited AMQP user">

# git repo
export GIT_REPO="https://github.com/akamai-compute-marketplace/marketplace-apps.git"
export WORK_DIR="/tmp/marketplace-apps" 
export MARKETPLACE_APP="apps/linode-marketplace-rabbitmq"
...
EOF
  ...
  if [[ -z ${ALLOWED_IPS_AMQP} ]]; then
    echo "[info] No IP address provided for whitelisting"
  else
    echo "allowed_ips_amqp: [${ALLOWED_IPS_AMQP}]" >> ${group_vars}
  fi

  if [[ -n ${RABBITMQ_ADMIN_USERNAME} ]]; then
    echo "rabbitmq_admin_username: ${RABBITMQ_ADMIN_USERNAME}" >> ${group_vars}
  fi

  if [[ -n ${RABBITMQ_USERNAME} ]]; then
    echo "rabbitmq_username: ${RABBITMQ_USERNAME}" >> ${group_vars}
  fi

  if [[ -n ${RABBITMQ_VERSION} ]]; then
    echo "rabbitmq_version: ${RABBITMQ_VERSION}" >> ${group_vars}
  fi
}
