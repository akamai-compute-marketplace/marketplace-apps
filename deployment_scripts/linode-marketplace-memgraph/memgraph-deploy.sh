...
#<UDF name="memgraph_bolt_allow" label="IP addresses allowed to access Bolt" example="192.0.2.21/32, 198.51.100.17/24" default="">
...
export MARKETPLACE_APP="apps/linode-marketplace-memgraph"
...
EOF
  ...
  if [[ -z ${MEMGRAPH_BOLT_ALLOW} ]]; then
    echo "[info] No IP addresses provided for bolt whitelisting"
  else
    echo "memgraph_bolt_allow: [${MEMGRAPH_BOLT_ALLOW}]" >> ${group_vars}
  fi  
