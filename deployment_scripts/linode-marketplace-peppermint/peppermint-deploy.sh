...
function run {
  ...
  git -C /tmp clone --depth 1 --filter=blob:none ${GIT_REPO} --branch peppermint --sparse
  cd ${WORK_DIR}
  git sparse-checkout init --cone
  git sparse-checkout set apps/linode-marketplace-peppermint apps/linode_helpers
