...
## Unified Streaming origin Settings 
#<UDF name="usp_license_password" Label="License Key" />
#<UDF name="ui_port" Label="Port for the UI" example="8088" default="80"/>

# git repo
export GIT_REPO="https://github.com/jcotoBan/marketplace-apps.git"
export WORK_DIR="/tmp/marketplace-apps" 
export MARKETPLACE_APP="apps/linode-marketplace-unifiedStreaming"
...
function udf {
  ...
  #unifiedStreaming vars
  
  if [[ -n ${USP_LICENSE_PASSWORD} ]]; then
    echo "usp_license_password: ${USP_LICENSE_PASSWORD}" >> ${group_vars};
  fi

  if [[ -n ${UI_PORT} ]]; then
    echo "ui_port: ${UI_PORT}" >> ${group_vars};
  fi
