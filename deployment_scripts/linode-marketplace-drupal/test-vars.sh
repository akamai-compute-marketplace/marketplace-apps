...
if [[ -n "${DRUPAL_SITE_NAME}" ]]; then
        UDF_VARS["DRUPAL_SITE_NAME"]="${DRUPAL_SITE_NAME}"
else
        UDF_VARS["DRUPAL_SITE_NAME"]="My Drupal Site" # default
fi

if [[ -n "${DRUPAL_SITE_EMAIL}" ]]; then
        UDF_VARS["DRUPAL_SITE_EMAIL"]="${DRUPAL_SITE_EMAIL}"
else
        UDF_VARS["DRUPAL_SITE_EMAIL"]="drupal@${DEFAULT_DNS}" # default
fi

if [[ -n "${DRUPAL_ACCOUNT_EMAIL}" ]]; then
        UDF_VARS["DRUPAL_ACCOUNT_EMAIL"]="${DRUPAL_ACCOUNT_EMAIL}"
else
        UDF_VARS["DRUPAL_ACCOUNT_EMAIL"]="account@${DEFAULT_DNS}" # default
fi

if [[ -n "${DRUPAL_USERNAME}" ]]; then
        UDF_VARS["DRUPAL_USERNAME"]="${DRUPAL_USERNAME}"
else
        UDF_VARS["DRUPAL_USERNAME"]="drupaluser" # default
fi
