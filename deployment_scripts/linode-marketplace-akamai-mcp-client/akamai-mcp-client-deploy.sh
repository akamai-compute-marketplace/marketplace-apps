...
## Claude Settings
#<UDF name="llm_client" label="LLM Client" oneOf="Claude" default="Claude">

## Akamai MCP Client Settings
#<UDF name="akamai_mcp_url" label="Akamai MCP Gateway URL" oneOf="https://mcp.akamai.com/mcp" default="https://mcp.akamai.com/mcp">
#<UDF name="jwt_password" label="Akamai JWT token">
...
EOF
  ...
  if [ "$LLM_CLIENT" = "Claude" ]; then
    echo "llm_client: claude" >> ${group_vars}
  else
    echo "No valid LLM chosen"
  fi
