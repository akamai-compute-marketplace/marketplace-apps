...
if [[ -n "${JAEGER_ADMIN_USER}" ]]; then
        UDF_VARS["JAEGER_ADMIN_USER"]="${JAEGER_ADMIN_USER}"
else
        UDF_VARS["JAEGER_ADMIN_USER"]="jaegeradmin" # default
fi

if [[ -n "${JAEGER_ADMIN_EMAIL}" ]]; then
        UDF_VARS["JAEGER_ADMIN_EMAIL"]="${JAEGER_ADMIN_EMAIL}"
else
        UDF_VARS["JAEGER_ADMIN_EMAIL"]="jaeger@${DEFAULT_DNS}" # default
fi
