...
| GPU Utils | Detects GPU on the instance and installs NVIDIA drivers | Writes `gpu_device` and `_has_gpu` to `group_vars/linode/vars`. |
...
Open WebUI provides built-in support for Retrieval-Augmented Generation (RAG), allowing users to chat with their own documents. This deployment uses Nginx as a frontend proxy to the Open WebUI container.

Users uploading documents larger than 100 MB must update the Nginx configuration to increase `client_max_body_size`.

The Nginx virtual host configuration can be found at:
